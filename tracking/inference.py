...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            agentDomain.append((i, j))

    # longest manhatten distance X_RANGE+Y_RANGE-2 (+MAX_NOISE)
    for i in range(X_RANGE + Y_RANGE + MAX_NOISE - 1):
        distDomain.append(i)

    variableDomainsDict[PAC] = agentDomain
    variableDomainsDict[GHOST0] = agentDomain
    variableDomainsDict[GHOST1] = agentDomain
    variableDomainsDict[OBS0] = distDomain
    variableDomainsDict[OBS1] = distDomain
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    def inferenceByVariableElimination(
        ...
        # according to the order, join all factors by variable
        for hiddenVar in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(
                currentFactorsList, hiddenVar
            )
            # If a factor that you are about to eliminate a variable from has
            # only one unconditioned variable, you should not eliminate it
            ...
            if len(joinedFactor.unconditionedVariables()) > 1:
                ...
                currentFactorsList.append(eliminate(joinedFactor, hiddenVar))

        # get the final joint distribution
        jointDistribution = joinFactors(currentFactorsList)

        # normalize so that the probability sums to one
        # the input factor contains only the query variables and the evidence variables,
        # both as unconditioned variables
        queryConditionedOnEvidence = normalize(jointDistribution)
        # now the factor is conditioned on the evidence variables
...
class InferenceModule:
    ...
    def getObservationProb(
        ...
        if ghostPosition == jailPosition or noisyDistance == None:
            return ghostPosition == jailPosition and noisyDistance == None
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # get weight distribution, ghostPosition is evenly distributed in particles now
        for ghostPosition in self.particles:
            weightDistribution[ghostPosition] += self.getObservationProb(
                observation, pacmanPosition, ghostPosition, jailPosition
            )

        # handle special case : all weight == 0
        if abs(weightDistribution.total() - 0) < 1e-6:
            self.initializeUniformly(gameState)
            return
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        newParticles = []
        # for each particle, get its newPosDistribution, and then sample to get the new position
        for oldPos in self.particles:
            newParticles.append(
                self.getPositionDistribution(gameState, oldPos).sample()
            )
